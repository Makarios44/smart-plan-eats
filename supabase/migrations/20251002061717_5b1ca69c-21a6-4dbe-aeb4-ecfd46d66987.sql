...
AS $$
  SELECT EXISTS (
    SELECT nutritionist_id AS user_id FROM public.client_assignments WHERE nutritionist_id = _user_id AND organization_id = _org_id
    UNION
    SELECT client_id AS user_id FROM public.client_assignments WHERE client_id = _user_id AND organization_id = _org_id
    UNION
    SELECT user_id FROM public.organization_members WHERE user_id = _user_id AND organization_id = _org_id
  ) OR EXISTS (
    SELECT 1
    FROM public.user_roles
    WHERE user_id = _user_id
...
CREATE INDEX idx_client_assignments_nutritionist ON public.client_assignments(nutritionist_id);
CREATE INDEX idx_client_assignments_client ON public.client_assignments(client_id);
CREATE INDEX idx_client_assignments_org ON public.client_assignments(organization_id);
